...
class NKSimulation(GenotypePhenotypeMap):
    ...
    sub-sequences that make up the genotype using a sliding window across the full genotype
    ...
    def set_order(self, K):
        ...
        # point to order
        self.order = self.K
        self._keys = np.array(["".join(r) for r in it.product('01', repeat=self.K)])
        # Reset phenotypes
        self.phenotypes = np.empty(self.n, dtype=float)

    def set_random_values(self, k_range=(0,1)):
        """Set the values of the NK table by drawing from a uniform distribution
        between the given k_range.
        """
        if hasattr(self, "keys") is False:
            raise Exception("Need to set K first. Try `set_order` method." )
        self._values = np.random.uniform(k_range[0], k_range[1], size=len(self.keys))
        self.build()
    ...
    def set_table_values(self, values):
        ...
        if len(values) != len(self.keys):
            raise Exception("Length of the values do not equal the length of NK keys. "
